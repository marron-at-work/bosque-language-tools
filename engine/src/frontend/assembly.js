import { FullyQualifiedNamespace, LambdaTypeSignature, VoidTypeSignature, LambdaParameterSignature, AutoTypeSignature, NominalTypeSignature, TemplateNameMapper } from "./type.js";
import { ExpressionTag, AccessNamespaceConstantExpression, LiteralRegexExpression, AccessStaticFieldExpression } from "./body.js";
...
var TemplateTermDeclExtraTag;
(function (TemplateTermDeclExtraTag) {
    TemplateTermDeclExtraTag["KeyType"] = "keytype";
    TemplateTermDeclExtraTag["Numeric"] = "numeric";
    TemplateTermDeclExtraTag["Equiv"] = "equiv";
    TemplateTermDeclExtraTag["Mergeable"] = "mergeable";
})(TemplateTermDeclExtraTag || (TemplateTermDeclExtraTag = {}));
class TemplateTermDecl {
    constructor(name, tconstraint, extraTags) {
        this.name = name;
        this.tconstraint = tconstraint;
        this.extraTags = extraTags;
    }
    emitHelper() {
        let chks = this.extraTags.map((t) => t);
...
class TypeTemplateTermDecl extends TemplateTermDecl {
    constructor(name, tags, tconstraint) {
...
class InvokeTemplateTermDecl extends TemplateTermDecl {
    constructor(name, tags, tconstraint, caninfer) {
        super(name, tconstraint, tags);
...
class InvokeTemplateTypeRestrictionClause {
    constructor(t, subtype, extraTags) {
        this.t = t;
...
class AbstractDecl {
    constructor(file, sinfo) {
        this.file = file;
        this.sinfo = sinfo;
    }
}
class ConditionDecl extends AbstractDecl {
    constructor(file, sinfo, diagnosticTag) {
        super(file, sinfo);
...
class PreConditionDecl extends ConditionDecl {
    constructor(file, sinfo, tag, ii, level, issoft, exp) {
...
class InvariantDecl extends ConditionDecl {
    constructor(file, sinfo, ii, tag, level, exp) {
        super(file, sinfo, tag);
        this.ii = ii;
        this.level = level;
        this.exp = exp;
...
class ValidateDecl extends ConditionDecl {
    constructor(file, sinfo, ii, tag, exp) {
        super(file, sinfo, tag);
        this.ii = ii;
        this.exp = exp;
    }
...
class AbstractCoreDecl extends AbstractDecl {
    constructor(file, sinfo, attributes, name) {
        super(file, sinfo);
...
class InvokeParameterDecl {
    constructor(name, type, optDefaultValue, pkind, isRestParam) {
        this.name = name;
        this.type = type;
...
class AbstractInvokeDecl extends AbstractCoreDecl {
    constructor(file, sinfo, attributes, name, recursive, params, resultType, body) {
...
class LambdaDecl extends AbstractInvokeDecl {
    constructor(file, sinfo, attributes, name, recursive, params, resultType, body, isAuto) {
    ...
    generateSig(sinfo) {
        ...
        return new LambdaTypeSignature(sinfo, this.recursive, this.name, lpsigs, this.resultType);
    }
    emitSig(fmt) {
        const attrs = this.emitAttributes();
        let rec = "";
...
class ExplicitInvokeDecl extends AbstractInvokeDecl {
    constructor(file, sinfo, attributes, name, recursive, params, resultType, body, terms, termRestriction, preconditions, postconditions) {
        super(file, sinfo, attributes, name, recursive, params, resultType, body);
        this.terms = terms;
    ...
    emit(fmt) {
        ...
        return fmt.indent(`${ssig[0]} ${ttinfo} ${restricts}${this.name}${terms}${ssig[1]} ${this.body.emit(fmt, meta)}`);
    }
}
class FunctionInvokeDecl extends ExplicitInvokeDecl {
    constructor(file, sinfo, attributes, name, recursive, params, resultType, body, terms, termRestriction, preconditions, postconditions) {
...
class NamespaceFunctionDecl extends FunctionInvokeDecl {
    constructor(file, sinfo, attributes, name, recursive, params, resultType, body, terms, termRestriction, preconditions, postconditions, tassoc, fkind) {
        super(file, sinfo, attributes, name, recursive, params, resultType, body, terms, termRestriction, preconditions, postconditions);
        this.fkind = fkind;
        this.tassoc = tassoc;
    }
...
class MethodDecl extends ExplicitInvokeDecl {
    constructor(file, sinfo, attributes, name, recursive, params, resultType, body, terms, termRestriction, preconditions, postconditions, isThisRef) {
        super(file, sinfo, attributes, name, recursive, params, resultType, body, terms, termRestriction, preconditions, postconditions);
...
class TaskMethodDecl extends ExplicitInvokeDecl {
    constructor(file, sinfo, attributes, name, recursive, params, resultType, body, terms, termRestriction, preconditions, postconditions, isSelfRef) {
        super(file, sinfo, attributes, name, recursive, params, resultType, body, terms, termRestriction, preconditions, postconditions);
...
class TaskActionDecl extends ExplicitInvokeDecl {
    constructor(file, sinfo, attributes, name, params, resultType, body, terms, termRestriction, preconditions, postconditions) {
...
class ConstMemberDecl extends AbstractCoreDecl {
    constructor(file, sinfo, attributes, name, dtype, value) {
        super(file, sinfo, attributes, name);
...
class MemberFieldDecl extends AbstractCoreDecl {
    constructor(file, sinfo, attributes, name, dtype, dvalue, isSpecialAccess) {
        super(file, sinfo, attributes, name);
...
var AdditionalTypeDeclTag;
(function (AdditionalTypeDeclTag) {
    AdditionalTypeDeclTag[AdditionalTypeDeclTag["Std"] = 0] = "Std";
    AdditionalTypeDeclTag[AdditionalTypeDeclTag["Status"] = 1] = "Status";
    AdditionalTypeDeclTag[AdditionalTypeDeclTag["Event"] = 2] = "Event";
})(AdditionalTypeDeclTag || (AdditionalTypeDeclTag = {}));
class AbstractNominalTypeDecl extends AbstractDecl {
    constructor(file, sinfo, attributes, ns, name, etag) {
        super(file, sinfo);
        this.terms = [];
        this.provides = [];
        this.invariants = [];
        ...
        this.functions = [];
        this.methods = [];
        this.saturatedProvides = [];
        this.saturatedBFieldInfo = [];
        this.allInvariants = [];
        this.allValidates = [];
        this.hasDynamicInvokes = false;
        this.attributes = attributes;
        this.ns = ns;
        this.name = name;
        this.etag = etag;
    ...
    isSpecialResultEntity() { return (this instanceof OkTypeDecl) || (this instanceof FailTypeDecl); }
    isSpecialAPIResultEntity() { return (this instanceof APIErrorTypeDecl) || (this instanceof APIRejectedTypeDecl) || (this instanceof APIDeniedTypeDecl) || (this instanceof APIFlaggedTypeDecl) || (this instanceof APISuccessTypeDecl); }
    hasAttribute(aname) {
        return this.attributes.find((attr) => attr.name === aname) !== undefined;
    }
    emitAttributes() {
    ...
    joinBodyGroups(groups) {
        return groups.map((g) => g.join("\n")).join("\n\n");
    }
}
class AbstractEntityTypeDecl extends AbstractNominalTypeDecl {
    constructor(file, sinfo, attributes, ns, name, etag) {
        super(file, sinfo, attributes, ns, name, etag);
    }
}
class EnumTypeDecl extends AbstractEntityTypeDecl {
    constructor(file, sinfo, attributes, ns, name, members, etag) {
        super(file, sinfo, attributes, ns, name, etag);
...
class TypedeclTypeDecl extends AbstractEntityTypeDecl {
    constructor(file, sinfo, attributes, ns, name, etag, valuetype) {
        super(file, sinfo, attributes, ns, name, etag);
...
class InternalEntityTypeDecl extends AbstractEntityTypeDecl {
    constructor(file, sinfo, attributes, name) {
...
class ConstructableTypeDecl extends InternalEntityTypeDecl {
    constructor(file, sinfo, attributes, name) {
        super(file, sinfo, attributes, name);
    }
}
...
class AbstractCollectionTypeDecl extends InternalEntityTypeDecl {
    constructor(file, sinfo, attributes, name) {
        super(file, sinfo, attributes, name);
    }
    emit(fmt) {
...
class AbstractConceptTypeDecl extends AbstractNominalTypeDecl {
    constructor(file, sinfo, attributes, ns, name, etag) {
        super(file, sinfo, attributes, ns, name, etag);
    }
}
class InternalConceptTypeDecl extends AbstractConceptTypeDecl {
    constructor(file, sinfo, attributes, name) {
        super(file, sinfo, attributes, new FullyQualifiedNamespace(["Core"]), name, AdditionalTypeDeclTag.Std);
    }
}
...
class ResultTypeDecl extends InternalConceptTypeDecl {
    constructor(file, sinfo, attributes, name) {
        super(file, sinfo, attributes, name);
        this.nestedEntityDecls = [];
    }
    getOkType() {
        return this.nestedEntityDecls.find((ned) => ned instanceof OkTypeDecl);
    }
    getFailType() {
        return this.nestedEntityDecls.find((ned) => ned instanceof FailTypeDecl);
    }
    emit(fmt) {
        const attrs = this.emitAttributes();
        fmt.indentPush();
...
class APIResultTypeDecl extends InternalConceptTypeDecl {
    ...
    getAPIErrorType() {
        return this.nestedEntityDecls.find((ned) => ned instanceof APIErrorTypeDecl);
    }
    getAPIRejectedType() {
        return this.nestedEntityDecls.find((ned) => ned instanceof APIRejectedTypeDecl);
    }
    getAPIDeniedType() {
        return this.nestedEntityDecls.find((ned) => ned instanceof APIDeniedTypeDecl);
    }
    getAPIFlaggedType() {
        return this.nestedEntityDecls.find((ned) => ned instanceof APIFlaggedTypeDecl);
    }
    getAPISuccessType() {
        return this.nestedEntityDecls.find((ned) => ned instanceof APISuccessTypeDecl);
    }
    emit(fmt) {
        const attrs = this.emitAttributes();
        fmt.indentPush();
...
class DatatypeMemberEntityTypeDecl extends AbstractEntityTypeDecl {
    constructor(file, sinfo, attributes, ns, name, etag, parentTypeDecl) {
        super(file, sinfo, attributes, ns, name, etag);
        this.fields = [];
...
class DatatypeTypeDecl extends AbstractConceptTypeDecl {
    constructor(file, sinfo, attributes, ns, name, etag) {
        super(file, sinfo, attributes, ns, name, etag);
        this.fields = [];
        this.associatedMemberEntityDecls = [];
    }
    emit(fmt) {
        fmt.indentPush();
        const bg = this.emitBodyGroups(fmt);
...
class ResourceInformation {
    constructor(pathglobs) {
        this.pathglobs = pathglobs;
    }
    static emitSingleRInfo(fmt, pg, optas) {
        if (optas === undefined) {
            if (pg.length === 1) {
...
class APIDecl extends AbstractCoreDecl {
    constructor(file, sinfo, attributes, name, params, resultType, eventType, preconds, postconds, configs, statusinfo, envreqs, resourcereqs, body) {
        super(file, sinfo, attributes, name);
        this.params = params;
        this.resultType = resultType;
...
class TaskDecl extends AbstractNominalTypeDecl {
    constructor(file, sinfo, attributes, ns, name) {
        super(file, sinfo, attributes, ns, name, AdditionalTypeDeclTag.Std);
        this.fields = [];
        this.selfmethods = [];
        this.actions = [];
        this.configs = new TaskConfiguration(undefined, undefined, undefined);
        this.statusinfo = [];
        this.envreqs = [];
        this.resourcereqs = new ResourceInformation([]);
        this.eventinfo = [];
    }
    emit(fmt) {
        const attrs = this.emitAttributes();
...
class NamespaceDeclaration {
    constructor(name, topnamespace, fullnamespace) {
        this.usings = [];
        this.declaredNames = new Set();
        this.declaredTypeNames = []; //types, typedecls, tasks, agents
        this.declaredSubNSNames = new Set();
        this.declaredConstNames = new Set();
        this.declaredFunctionNames = new Set();
        this.declaredAPINames = new Set();
        this.declaredAgentNames = new Set();
        this.subns = [];
        this.consts = [];
        this.functions = []; //function as name+TemplateFlag+refFlag+lambdaFlag
        this.typedecls = [];
        this.apis = [];
        this.tasks = [];
        this.agents = [];
    ...
    checkDeclNameClashType(rname, hasterms) {
        if (!this.declaredNames.has(rname)) {
            return false;
        }
    ...
    checkDeclNameClashMemberSimple(rname) {
...
class Assembly {
    ...
    hasToplevelNamespace(ns) {
        return this.toplevelNamespaces.find((nsd) => nsd.name === ns) !== undefined;
    }
    getCoreNamespace() {
        return this.toplevelNamespaces.find((nsd) => nsd.name === "Core");
    }
    getToplevelNamespace(ns) {
        return this.toplevelNamespaces.find((nsd) => nsd.name === ns);
    }
    ensureToplevelNamespace(ns) {
    ...
    resolveConstantRegexExpressionValue(exp, inns) {
    ...
    resolveFieldDeclDefaultExpFromTypeSignature(ts, fieldname) {
    ...
    static checkFunctionSigMatch(fd1, fd2) {
    ...
    static checkMethodSigMatch(md1, md2) {
    ...
    resolveNamespaceConstant(ns, name) {
    ...
    resolveNamespaceFunction(ns, name, isTemplate, hasLambda, isRef) {
        const nsdecl = this.resolveNamespaceDecl(ns.ns);
        if (nsdecl === undefined) {
            return undefined;
        }
    ...
    tryReduceConstantExpressionToRE(exp) {
        if (exp instanceof LiteralRegexExpression) {
            return exp;
        }
    ...
    loadConstantsAndValidatorREs(nsdecl) {
        const inns = nsdecl.fullnamespace.ns.join("::");
        const nsmappings = nsdecl.usings.filter((u) => u.asns !== undefined).map((u) => [u.fromns, u.asns]);
        const nsinfo = { inns: inns, nsmappings: nsmappings };
        const reinfos = [];
    ...
    tryReduceConstantExpression(exp, currentBinds) {
        if (exp.isLiteralExpression()) {
            ...
            const tsdecltype = ((currentBinds !== undefined) ? exp.resolvedDeclType.remapTemplateBindings(currentBinds) : exp.resolvedDeclType);
...
export { MIN_SAFE_INT, MAX_SAFE_INT, MAX_SAFE_NAT, MIN_SAFE_CHK_INT, MAX_SAFE_CHK_INT, MAX_SAFE_CHK_NAT, WELL_KNOWN_RETURN_VAR_NAME, WELL_KNOWN_EVENTS_VAR_NAME, TemplateTermDeclExtraTag, TemplateTermDecl, TypeTemplateTermDecl, InvokeTemplateTermDecl, InvokeTemplateTypeRestrictionClause, InvokeTemplateTypeRestriction, TaskConfiguration, AbstractDecl, ConditionDecl, PreConditionDecl, PostConditionDecl, InvariantDecl, ValidateDecl, DeclarationAttibute, AbstractCoreDecl, InvokeParameterDecl, AbstractInvokeDecl, LambdaDecl, ExplicitInvokeDecl, TestAssociation, FunctionInvokeDecl, NamespaceFunctionDecl, TypeFunctionDecl, MethodDecl, TaskMethodDecl, TaskActionDecl, ConstMemberDecl, MemberFieldDecl, AbstractNominalTypeDecl, AdditionalTypeDeclTag, EnumTypeDecl, TypedeclTypeDecl, AbstractEntityTypeDecl, PrimitiveEntityTypeDecl, InternalEntityTypeDecl, ConstructableTypeDecl, OkTypeDecl, FailTypeDecl, APIErrorTypeDecl, APIRejectedTypeDecl, APIDeniedTypeDecl, APIFlaggedTypeDecl, APISuccessTypeDecl, SomeTypeDecl, MapEntryTypeDecl, AbstractCollectionTypeDecl, ListTypeDecl, StackTypeDecl, QueueTypeDecl, SetTypeDecl, MapTypeDecl, EventListTypeDecl, EntityTypeDecl, AbstractConceptTypeDecl, InternalConceptTypeDecl, OptionTypeDecl, ResultTypeDecl, APIResultTypeDecl, ConceptTypeDecl, DatatypeMemberEntityTypeDecl, DatatypeTypeDecl, EnvironmentVariableInformation, ResourceInformation, APIDecl, AgentDecl, TaskDecl, NamespaceConstDecl, NamespaceUsing, NamespaceDeclaration, Assembly };
