...
import { LayoutTag, LIST_T_INLINE_CAPACITY, LIST_T_LEAF_CAPACITY, TypeInfoManager } from "./typeinfomgr.js";
import { MAX_SAFE_INT, MAX_SAFE_NAT, MIN_SAFE_INT } from "../../frontend/assembly.js";
import { IRExpressionTag, IRLiteralExpression, IRImmediateExpression, IRSimpleExpression, IRAtomicStatement, IRStatementTag, IRBuiltinBody, IRHoleBody, IRSimpleIfStatement, IRSimpleIfElseStatement, IRFormatStringTextComponent, IRMatchExactStatement, IRMatchGeneralStatement, IRBlockStatement } from "../irdefs/irbody.js";
import { IRAbstractCollectionTypeDecl, IRAbstractEntityTypeDecl, IRAPIResultTypeDecl, IRConceptTypeDecl, IRConstructableTypeDecl, IRDatatypeMemberEntityTypeDecl, IRDatatypeTypeDecl, IREntityTypeDecl, IREnumTypeDecl, IRFailTypeDecl, IRInternalConceptTypeDecl, IRListTypeDecl, IRMapEntryTypeDecl, IRMapTypeDecl, IROkTypeDecl, IROptionTypeDecl, IRPrimitiveEntityTypeDecl, IRResultTypeDecl, IRSomeTypeDecl, IRTypedeclCStringDecl, IRTypedeclStringDecl, IRTypedeclTypeDecl } from "../irdefs/irassembly.js";
import { IRDashResultTypeSignature, IREListTypeSignature, IRFormatCStringTypeSignature, IRFormatStringTypeSignature, IRFormatTypeSignature, IRLambdaParameterPackTypeSignature, IRNominalTypeSignature, IRVoidTypeSignature } from "../irdefs/irtype.js";
...
class CPPEmitter {
    constructor(irasm, typeInfoManager) {
        //The C++ TaskInfoRepr<U> for accessing the global info for the task we are emitting
        this.cppTaskType = undefined;
        this.irasm = irasm;
        this.typeInfoManager = typeInfoManager;
    }
    static emitNumericRangeBound(bound) {
        const suffixMap = { "i": "_i", "n": "_n", "I": "_I", "N": "_N", "f": "_f", "d": "_d", "R": "_R" };
    ...
    escapeLiteralCString(cstrbytes) {
        let escstr = '"';
    ...
    isNominalTypeUniqueEntity(t) {
        if (!(t instanceof IRNominalTypeSignature)) {
            return false;
        }
        const decl = this.irasm.alltypes.get(t.tkeystr);
        return decl instanceof IRAbstractEntityTypeDecl;
    }
    getAllConcreteSubtypeOptionsForNominalType(t) {
        return this.irasm.concretesubtypes.get(t.tkeystr)
            .map((st) => this.irasm.alltypes.get(st.tkeystr));
    }
    emitIRLiteral(exp) {
        const ttag = exp.tag;
        if (ttag === IRExpressionTag.IRLiteralNoneExpression) {
            ...
            if (ll === "ChkNat::npos") {
                ...
                if (nval <= MAX_SAFE_NAT) {
                    ...
                    assert(false, `CPPEmitter: need to do bit shift construction for (really big) safe nat -- ${exp.value}`);
                }
            }
        }
            ...
            if (ll === "ChkInt::npos") {
                ...
                if (MIN_SAFE_INT <= ival && ival <= MAX_SAFE_INT) {
                    ...
                    assert(false, `CPPEmitter: need to do bit shift construction for (really big) safe int -- ${exp.value}`);
                }
            }
        }
            ...
            return `${RUNTIME_NAMESPACE}::XComplex(${exp.real}, ${exp.imaginary})`;
        }
            ...
            const bytes = exp.bytes.map((b) => `0x${b.toString(16).padStart(2, '0')}`).join(", ");
            ...
            const [escstr, lcount] = this.escapeLiteralCString(exp.bytes);
            ...
            const [ustr, lcount] = this.escapeLiteralString(exp.bytes);
            ...
            return `${cce}{${this.emitIRLiteral(ilte.value)}}`;
        }
    ...
    emitIRImmediateExpression(exp) {
        if (exp instanceof IRLiteralExpression) {
            ...
            if (ttag === IRExpressionTag.IRAccessConstantExpression) {
                return TransformCPPNameManager.generateNameForConstantKey(exp.constkey) + "()";
            }
            else if (ttag === IRExpressionTag.IRAccessEnumExpression) {
                return TransformCPPNameManager.generateNameForEnumKey(exp.tkey, exp.membername);
            }
            else if (ttag === IRExpressionTag.IRAccessParameterVariableExpression) {
                return TransformCPPNameManager.convertIdentifier(exp.pname);
            }
            else if (ttag === IRExpressionTag.IRAccessLocalVariableExpression) {
                return TransformCPPNameManager.convertIdentifier(exp.vname);
            }
    ...
    emitIRSimpleExpression(exps, toplevel) {
        if (exps instanceof IRImmediateExpression) {
            ...
            if (ttag === IRExpressionTag.IRAccessTypeDeclValueExpression) {
                ...
                bstr = `!${this.emitIRSimpleExpression(exps.exp, false)}`;
            }
            else if (ttag === IRExpressionTag.IRPrefixPlusOpExpression) {
                bstr = `${this.emitIRSimpleExpression(exps.exp, toplevel)}`;
            }
                ...
                if (this.isNominalTypeUniqueEntity(afse.eexptype)) {
                    ...
                    const allsubs = this.getAllConcreteSubtypeOptionsForNominalType(afse.eexptype);
    ...
    emitExpression(exp, toplevel) {
        if (exp instanceof IRSimpleExpression) {
            ...
            if (ttag === IRExpressionTag.IRAccessEnvHasExpression) {
                ...
                const ldecl = this.irasm.alltypes.get(iclse.constype.tkeystr);
    ...
    emitIRSimpleIfStatement(ifstmt, isfinal, indent) {
    ...
    emitIRSimpleIfElseStatement(ifstmt, isfinal, indent) {
        const ttest = this.emitIRSimpleExpression(ifstmt.cond, true);
        const bindent = indent !== undefined ? indent + "    " : undefined;
        const tbody = this.emitStatementList(ifstmt.tblock.statements, undefined, undefined, bindent);
    ...
    emitIRMatchExactStatement(mstmt, isfinal, indent) {
    ...
    emitIRMatchGeneralStatement(mstmt, isfinal, indent) {
        const sval = this.emitIRImmediateExpression(mstmt.sval);
        ...
        const ops = mstmt.matchflow.map((mf, ii) => {
            ...
            if (mf.mtype === undefined || ii === mstmt.matchflow.length - 1) {
                ...
                const mftinfo = this.irasm.alltypes.get(mf.mtype.tkeystr);
    ...
    emitBlockStatementInline(stmt, prefx, postfix, indent) {
        return this.emitStatementList(stmt.statements, prefx, postfix, indent);
    }
    emitStatement(stmt, isfinal, indent) {
    ...
    emitStatementList(stmts, prefx, postfix, indent) {
    ...
    */
    getParamInforForLambda(invk, lname) {
        const lparam = invk.params.find((p) => p.name === lname);
        const lpptype = this.irasm.alllambdas.get(lparam.type.tkeystr);
        const fikey = lpptype.invtrgt;
        const ll = this.irasm.invokes.find((inv) => inv.ikey === fikey);
        const llpp = ll.params.slice(1).map((p) => this.typeInfoManager.emitTypeAsParameter(p.type.tkeystr, p.pkind !== undefined, false) + " " + TransformCPPNameManager.convertIdentifier(p.name));
    ...
    emitBuiltinBody(invk, body, indent) {
        let prestr = "";
        let bstr;
        if (body.builtin === "float_is_safe_convert_into") {
            const intotype = body.biterms.find((bt) => bt[0] === "T");
            ...
            const jtype = body.biterms.find((bt) => bt[0] === "J");
            const rtype = this.typeInfoManager.getTypeInfo(invk.resultType.tkeystr);
            ...
            const utype = body.biterms.find((bt) => bt[0] === "U");
    ...
    emitPreconditionCheckFunction(ipcs, invk) {
    ...
    emitPostconditionCheckFunction(ipcs, invk, haspassing) {
        ...
        if (!haspassing) {
            ...
            const pparam = invk.params.find((p) => p.pkind !== undefined);
    ...
    emitInvariantFunction(iinv, tdecl, pinfo) {
    ...
    emitValidateFunction(ival, tdecl, pinfo) {
    ...
    emitConstantDeclInfo(iconst) {
        ...
        const bodystr = this.emitStatementList(iconst.stmts, [`if(${gvname}.has_value()) { return *(${gvname}.value()); }`], [
    ...
    emitIRInvokeParameterDecl(iparam) {
    ...
    emitListTypeInfoDecl(tdecl) {
        ...
        let inlinemask = undefined;
        let leafmask = undefined;
        let nodemask = undefined;
        if (!/[1-5]/.test(eemask)) {
    ...
    emitEntityTypeInfoWForwarDecl(tdecl) {
        const ctname = TransformCPPNameManager.convertTypeKey(tdecl.tkey);
        const ttid = this.typeInfoManager.getTypeInfo(tdecl.tkey);
        const superlist = this.irasm.concretesupertypes.get(tdecl.tkey).map((tt) => this.typeInfoManager.getTypeInfo(tt.tkeystr).bsqtypeid).sort();
        let superdecl = "";
    ...
    emitEntityTypeInfoWAllocInfo(tdecl) {
        const ctname = TransformCPPNameManager.convertTypeKey(tdecl.tkey);
        const ttid = this.typeInfoManager.getTypeInfo(tdecl.tkey);
        const superlist = this.irasm.concretesupertypes.get(tdecl.tkey).map((tt) => this.typeInfoManager.getTypeInfo(tt.tkeystr).bsqtypeid).sort();
        let superdecl = "";
    ...
    emitRegexInfos(cregexs, uregexs) {
    ...
    emitGeneralTypeDeclInfo(tdecl, chkextra) {
        const ctname = TransformCPPNameManager.convertTypeKey(tdecl.tkey);
    ...
    emitCStringTypeDeclInfo(tcstr) {
        let echks = [];
        if (tcstr.rngchk !== undefined) {
            if (tcstr.rngchk.min === undefined) {
                echks.push(`if(${tcstr.rngchk.max.slice(0, -1)} < vv.size()) { return std::nullopt; };`);
            }
            else if (tcstr.rngchk.max === undefined) {
                echks.push(`if(vv.size() < ${tcstr.rngchk.min.slice(0, -1)}) { return std::nullopt; };`);
            }
            else {
                echks.push(`if((vv.size() < ${tcstr.rngchk.min.slice(0, -1)}) || (${tcstr.rngchk.max.slice(0, -1)} < vv.size())) { return std::nullopt; };`);
            }
        }
    ...
    emitResultTypeInfo(tdecl) {
        assert(false, "CPPEmitter: need to implement result type decl emission");
    }
    emitStdCommonEntityTypeInfo(tdecl, tlinfo, isRef) {
        const ctname = TransformCPPNameManager.convertTypeKey(tdecl.tkey);
        const ctrepr = this.typeInfoManager.emitTypeAsStd(tdecl.tkey);
        ...
        if (vfuncinfo.length === 0 && valfuncinfo.length === 0) {
            ...
            const allchks = [
                ...tdecl.allInvariants.map((inv) => {
                    const cttdecl = this.irasm.alltypes.get(inv.containingtype.tkeystr);
                ...
                ...tdecl.allValidates.map((val) => {
                    const cttdecl = this.irasm.alltypes.get(val.containingtype.tkeystr);
                    const aargs = cttdecl.saturatedBFieldInfo.map((bf) => {
                        const aidx = tdecl.saturatedBFieldInfo.findIndex((fb) => fb.fkey === bf.fkey);
                        assert(aidx !== -1);
                        return consargs[aidx];
    ...
    emitDatatypeMemberEntityTypeDecl(tdecl) {
        const tlayout = this.typeInfoManager.getTypeInfo(tdecl.tkey);
        if (tlayout.tag === LayoutTag.Value) {
            return this.emitStdValueEntityTypeInfo(tdecl, tlayout);
        }
    ...
    emitStdConceptTypeInfo(tdecl) {
        const ctname = TransformCPPNameManager.convertTypeKey(tdecl.tkey);
        const uctname = TransformCPPNameManager.generateNameForUnionType(tdecl.tkey);
        const uoptions = this.irasm.concretesubtypes.get(tdecl.tkey);
    ...
    emitConceptTypeInfo(tdecl) {
        return this.emitStdConceptTypeInfo(tdecl);
    }
    emitDatatypeTypeDeclInfo(tdecl) {
        return this.emitStdConceptTypeInfo(tdecl);
    }
    emitFormatTypeInfo(tdecl) {
    ...
    emitLambdaParameterPackTypeInfo(tdecl) {
        const ctname = TransformCPPNameManager.convertTypeKey(tdecl.tkeystr);
        const ddecl = this.irasm.alllambdas.get(tdecl.tkeystr);
    ...
    emitTypeDeclInfo() {
        ...
        const sccftypeinfos = this.irasm.typedepcycles.flatMap((cycle) => {
            const etypes = cycle.filter((ct) => this.typeInfoManager.getTypeInfo(ct.tkeystr).tag === LayoutTag.Ref);
            return etypes.map((ct) => this.emitEntityTypeInfoWForwarDecl(this.irasm.alltypes.get(ct.tkeystr)));
        });
        let sccfdecls = [];
        ...
        const decltdd = [...allntypes, ...allftypes]
            ...
            if (!(ttd instanceof IRNominalTypeSignature)) {
                ...
                const ctd = this.irasm.alltypes.get(ttd.tkeystr);
    ...
    emitParseArgsMain(idecl) {
        if (idecl.params.length === 0) {
            return '    //No args';
        }
        else {
            const initforparse = '    auto iobb = ᐸRuntimeᐳ::g_alloc_info.io_buffer_alloc();\n' +
            ...
            const finalizeparse = '    if(!ᐸRuntimeᐳ::tl_bosque_info.current_task->bsqparser.allInputConsumed()) { printf("Error parsing input -- invaliad data in tail of input\\n"); exit(1); }\n' +
    ...
    emitCommandLineMain(ikey) {
        const idecl = this.irasm.invokes.find((v) => v.ikey === ikey);
        const mmain = this.emitMMain(idecl);
        const sallocs = [
            ...this.irasm.primitives.map((pdcl) => this.typeInfoManager.generateAllocatorNameForTypeKeySpecialMapEntry(pdcl.tkey)).filter((aa) => aa !== undefined),
            ...this.irasm.collections.map((cdcl) => this.typeInfoManager.generateAllocatorNameForTypeKeySpecialMapEntry(cdcl.tkey)).filter((aa) => aa !== undefined),
            ...this.irasm.eventlists.map((ccdl) => this.typeInfoManager.generateAllocatorNameForTypeKeySpecialMapEntry(ccdl.tkey)).filter((aa) => aa !== undefined),
        ].flat();
        const allocs = [
            ...this.irasm.entities.map((edcl) => this.typeInfoManager.generateAllocatorNameForTypeKeyGeneralMapEntry(edcl.tkey)).filter((aa) => aa !== undefined),
            ...this.irasm.datamembers.map((cdcl) => this.typeInfoManager.generateAllocatorNameForTypeKeyGeneralMapEntry(cdcl.tkey)).filter((aa) => aa !== undefined)
        ];
        const initializegc = '{\n' +
    ...
    emitInvokeForKey(ikey) {
        const invk = this.irasm.invokes.find((v) => v.ikey === ikey);
        return this.emitIRInvokeDeclInfo(invk)[1];
    }
    emitForCommandLine(ikey) {
...
export { CPPEmitter };
//# sourceMappingURL=cppemit.js.map