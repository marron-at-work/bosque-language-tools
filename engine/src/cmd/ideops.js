...
const g_corecode = workflowLoadCoreSrc();
class IDEOpErrorInfo {
    constructor(filename, line, msg) {
        this.filename = filename;
        this.line = line;
        this.msg = msg;
    }
    static fromParserError(err) {
        return new IDEOpErrorInfo(err.srcfile, err.sinfo.line, err.message);
    }
    static fromTypeError(err) {
        return new IDEOpErrorInfo(err.file, err.line, err.msg);
    }
}
function runIDETypeCheck(usercode) {
    try {
...
process.exit(0);
//# sourceMappingURL=ideops.js.map